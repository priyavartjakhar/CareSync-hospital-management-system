...
## 🌐 Free Hosting Guide (Complete Full-Stack on Single Platform)

You can host the **complete CareSync stack (Frontend + Backend + Database)** 100% free on a single web hosting platform using **Render.com**.

### Render.com (Free Web Service Deployment)

Render offers a **Free Web Service** tier that can serve both the Python Flask Backend and built Vue 3 static assets from a single service container with zero extra costs.

#### Step-by-Step Render Deployment:

1. **Prepare Build Command in Repository**:
   Create a single build script (`build.sh`) in your root repository:
   ```bash
   #!/usr/bin/env bash
   set -o errexit
...
2. **Configure Flask to Serve Frontend Assets**:
   Update `backend/app.py` to serve static assets from `frontend/dist`:
   ```python
...
Your complete application (Frontend, Backend, Database) will be live on a free HTTPS URL like `https://caresync-hms.onrender.com` at **$0/month**.
